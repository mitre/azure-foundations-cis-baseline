control 'azure-foundations-cis-7.1' do
  ...
  desc 'fix',
       "Where RDP is not explicitly required and narrowly configured for resources attached to the Network Security Group, Internet-level access to your Azure resources should be restricted or eliminated. For internal access to relevant resources, configure an encrypted network tunnel such as:
            ExpressRoute
            Site-to-site VPN
            Point-to-site VPN"
  ...
  query_results_json.each do |nsg|
    ...
    describe "NSG: #{nsg_name}" do
      it 'should should not return any unrestructed RDP rule' do
        ...
        failure_message = "Check #{nsg_name} NSG's RDP access, direction, protocol, destinationPortRange, and sourceAddressPrefix fields for proper configurations"
