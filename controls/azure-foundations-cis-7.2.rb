    ...
    failure_message = nsgs_with_insecure_rules.map do |nsg|
      ...
      rules_messages = insecure_rules.map do |rule|
        <<~RULE_DETAILS
          ...
          - Destination Port Range: #{rule['destinationPortRange']} (Matched Condition: #{'Destination Port Range is "22", "*", or matches /22/' if rule['destinationPortRange'] == '22' || rule['destinationPortRange'] == '*' || (rule['destinationPortRange'] =~ /22/)})
