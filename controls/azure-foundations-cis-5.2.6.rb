  ...
  if rg_sa_list.empty?
    ...
    resource_groups.each do |resource_group|
      ...
      servers.each do |server|
        ...
        config_script = <<-EOH
          $ErrorActionPreference = "Stop"
          Get-AzPostgreSqlConfiguration -ResourceGroupName "#{resource_group}" -ServerName "#{server_name}" -Name log_connections | ConvertTo-Json -Depth 10
        EOH

        config_pwsh = powershell(config_script)
        raise Inspec::Error, "The powershell output returned the following error:  #{config_pwsh.stderr}" if config_pwsh.exit_status != 0

        configuration = json(content: config_pwsh.stdout.strip).params

        failures << "#{resource_group}/#{server_name}" unless configuration['Value'].casecmp('on').zero?
      end
    end

    describe 'PostgreSQL single servers with log_connections not set to ON' do
      subject { failures }
      it { should be_empty }
    end
  end
