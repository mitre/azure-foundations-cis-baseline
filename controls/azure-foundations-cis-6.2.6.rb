control 'azure-foundations-cis-6.2.6' do
  ...
  activity_log_exists_delete_ss_script = %(
            Get-AzActivityLogAlert -SubscriptionId "#{subscription_id}"|
            where-object {$_.ConditionAllOf.Equal -match "Microsoft.Security/securitySolutions/delete"}|
            select-object Location,Name,Enabled,ResourceGroupName,ConditionAllOf
    )

  powershell_output = powershell(activity_log_exists_delete_ss_script)
  describe 'Ensure that the subscriptions output for the activity log alert rule for Deleting a Security Solution' do
