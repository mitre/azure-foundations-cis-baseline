  ...
  if rg_sa_list.empty?
    ...
    resource_groups.each do |resource_group|
      ...
      servers.each do |server|
        ...
        failures << "#{resource_group}/#{server_name}" unless configuration['Value'].to_i > 3
      end
    end

    describe 'PostgreSQL Flexible servers with logfiles.retention_days not greater than 3' do
      subject { failures }
      it { should be_empty }
    end
  end
